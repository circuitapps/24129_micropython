...
def _to16bits(LSB, MSB):  # CT - Added
    return((MSB << 8) | LSB)

def tuple2seconds(datetime):
    return time.mktime((datetime.year, datetime.month, datetime.day, datetime.hour, datetime.minute, datetime.second, datetime.weekday, 0, 0))
...
class DS3231(_BaseRTC):
    ...
    def temperature(self): # CT - Added
        # Read 2 bytes from temperature register on RTC device
        buffer = self.i2c.readfrom_mem(self.address,
                                self._TEMP_REGISTER, 2)
        
        # Assuming buffer[0] = MSB buffer[1] = LSB
        _val16bits = _to16bits(buffer[1], buffer[0])
        temp_degC = _val16bits / 256  # Converting by multiplying by scaling factor.
        return temp_degC
